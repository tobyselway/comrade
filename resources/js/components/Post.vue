    ...
    <div class="mt-6 bg-white rounded shadow w-2/3 overflow-hidden">
        <div class="flex flex-col p-4 pb-2">
            <div class="flex items-center">
                <div class="w-10">
                    <img :src="post.data.attributes.posted_by.data.attributes.profile_image.data.attributes.path" alt="User avatar" class="w-10 h-10 object-cover rounded-full">
                </div>
        ...
        <div class="flex justify-between items-center border-1 border-gray-400 m-3">
            <div class="flex justify-between items-center">
                <button class="flex justify-center items-center p-1 rounded-full text-gray-700 w-8 h-8 bg-gray-200 hover:bg-gray-300 active:bg-gray-400 focus:outline-none"
            ...
            <button class="flex justify-center items-center py-2 rounded-lg text-sm text-gray-700 w-32 focus:outline-none"
        ...
        <div v-if="comments" class="border-t border-gray-200 p-4">
            <div class="flex-1 relative flex items-center">
                <input v-model="commentBody" type="text" name="comment" placeholder="Write a comment" class="mb-2 w-full pl-4 py-3 pr-8 h-8 text-sm rounded-full bg-gray-200 border-2 border-gray-200 focus:border-blue-300 focus:outline-none">
            ...
            <div v-for="comment in post.data.attributes.comments.data">
                <div class="flex mt-3 items-center">
                    <div class="w-8">
                        <img :src="comment.data.attributes.commented_by.data.attributes.profile_image.data.attributes.path" alt="User avatar" class="w-8 h-8 object-cover rounded-full">
                    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            postComment() {
                this.$store.dispatch('commentPost', { body: this.commentBody, postId: this.post.data.post_id, postKey: this.$vnode.key });
                this.commentBody = '';
            }
        },
